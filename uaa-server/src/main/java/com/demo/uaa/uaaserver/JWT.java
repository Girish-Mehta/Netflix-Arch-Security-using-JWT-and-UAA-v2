...
public class JWT {
	...
	@PostMapping("/login/{type}/{user}")
	public String createJwtToken(@PathVariable String type, @PathVariable String user) {
		...
		claims.setSubject(type); // the subject/principal is whom the token is about
		claims.setClaim("UserEmail", user); // additional claims/attributes about the subject can be added
		List<String> groups = Arrays.asList("User", "Admin");
		claims.setStringListClaim("Admin", groups); // multi-valued claims work too and will end up as a JSON array
